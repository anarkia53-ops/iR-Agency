    ...
    if (webhookUrl) {
      ...
      try {
        ...
        if (!webhookResponse.ok) {
          console.error('Contact webhook failed with non-2xx status:', webhookResponse.status);
          return NextResponse.json(
        ...
        return NextResponse.json(
          { error: isTimeout ? 'Your request timed out during delivery. Please retry.' : 'Your request could not be delivered. Please retry.' },
          { status: 502 },
        );
      } finally {
      ...
      console.warn('Contact submission accepted without webhook forwarding.');
    }
